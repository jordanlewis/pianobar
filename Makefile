...
LIBWARDROBE_DIR=libwardrobe/src
LIBWARDROBE_SRC=${LIBWARDROBE_DIR}/wardrobe.c \
		${LIBWARDROBE_DIR}/md5.c
LIBWARDROBE_HDR=${LIBWARDROBE_DIR}/config.h \
		${LIBWARDROBE_DIR}/wardrobe.h
LIBWARDROBE_OBJ=${LIBWARDROBE_SRC:.c=.o}
LIBWARDROBE_RELOBJ=${LIBWARDROBE_SRC:.c=.lo}
LIBWARDROBE_INCLUDE=${LIBWARDROBE_DIR}
...
pianobar: ${PIANOBAR_OBJ} ${PIANOBAR_HDR} ${LIBPIANO_OBJ} ${LIBWAITRESS_OBJ} \
		${LIBWAITRESS_HDR} ${LIBWARDROBE_OBJ} ${LIBWARDROBE_HDR} \
		${LIBEZXML_OBJ} ${LIBEZXML_HDR}
	${CC} ${CFLAGS} ${PIANOBAR_OBJ} ${LIBPIANO_OBJ} ${LIBWAITRESS_OBJ} \
			${LIBWARDROBE_OBJ} ${LIBEZXML_OBJ} ${LIBAO_LIB} ${LIBFAAD_LDFLAGS} ${LIBMAD_LDFLAGS} \
...
libpiano: ${LIBPIANO_RELOBJ} ${LIBPIANO_HDR} ${LIBWAITRESS_RELOBJ} \
	...
	${CC} -shared ${CFLAGS} ${LIBPIANO_RELOBJ} ${LIBWAITRESS_RELOBJ} 
			${LIBWARDROBE_RELOBJ} ${LIBEZXML_RELOBJ} -o $@.so.0.0.0

%.o: %.c
	${CC} ${CFLAGS} -I ${LIBPIANO_INCLUDE} -I ${LIBWAITRESS_INCLUDE} \
			-I ${LIBWARDROBE_INCLUDE} -I ${LIBEZXML_INCLUDE} -I ${LIBAO_INCLUDE} ${LIBFAAD_CFLAGS} \
			${LIBMAD_CFLAGS} -c -o $@ $<

# create position independent code (for shared libraries)
%.lo: %.c
	${CC} ${CFLAGS} -I ${LIBPIANO_INCLUDE} -I ${LIBWAITRESS_INCLUDE} \
			-I ${LIBWARDROBE_INCLUDE} \
			-I ${LIBEZXML_INCLUDE} -c -fPIC -o $@ $<

clean:
	${RM} ${PIANOBAR_OBJ} ${LIBPIANO_OBJ} ${LIBWAITRESS_OBJ} \
			${LIBWARDROBE_OBJ} ${LIBEZXML_OBJ} \
			${LIBPIANO_RELOBJ} ${LIBWAITRESS_RELOBJ} \
			${LIBWARDROBE_RELOBJ} ${LIBEZXML_RELOBJ} pianobar \
			libpiano.so.0.0.0
